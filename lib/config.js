...
function validate(config) {
    ...
    const groupsFormat = joi.array().min(1).items(joi.object({
        groupName: joi.string().min(1).required(),
        phrases: joi.array().items(
    ...
    const schema = joi.object({
        ...
        proxyHost: joi.string().min(1).default('webfilter'),
        ...
        networkConfig: joi.object({
            localTransparent: joi.boolean().default(false),
            routerConfig: joi.object().optional()
        }).default(),
        ...
        dnsIP: joi.string().min(1).default('dns'),
