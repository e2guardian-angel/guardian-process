...
function GuardianProcess() {
    ...
    this.user = process.env.GUARDIAN_USER || os.userInfo();
    this.volumePath = path.join('/home', this.user, '.volumes');
...
GuardianProcess.prototype.update = async function() {
    ...
    this.controller.initNetwork(config);
    ...
    // Deploy DNS
    await this.controller.deployDNSReverse();
    await this.controller.deployFilter();
    await this.controller.deployDNSFwd();
    ...
    await this.controller.deployNetwork(config);
