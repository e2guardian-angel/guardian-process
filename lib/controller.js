...
function Controller(kubeConfig, userInfo) {
    ...
    this.resources = {
        loaded: false,
        pods: {
            transocks: readResourceFile(`${__dirname}/json/transocks-pod.json`),
            dns: readResourceFile(`${__dirname}/json/dns-pod.json`)
        },
        deployments: {
...
Controller.prototype.generatePaths = function(namespace) {
    ...
    this.paths.resources.pods = {
        transocks: `${this.paths.kube.pods}/transocks`,
        dns: `${this.paths.kube.pods}/dns`
    };
...
Controller.prototype.renderAuthVolume = function(path) {
    ...
    authPv.spec.nodeAffinity.required.nodeSelectorTerms[0].matchExpressions[0].values[0] = this.nodeName;
    return {
        pv: authPv,
        pvc: authPvc
    };
...
Controller.prototype.deployFilter = async function() {
    //const podResource = this.resources.pods.transocks;
    ...
    try {
        ...
        /*await this.deployPod(
            this.paths.kube.pods,
            this.paths.resources.pods.transocks,
            podResource
        );*/
    } catch (err) {
...
Controller.prototype.deployPod = async function(kubePath, resourcePath, resource) {
    try {
        // If it exists, no need to deploy
        await this.kubeGet(resourcePath);
    } catch (err) {
        // 404; create
        await this.kubePost(kubePath, resource);
    }
}

Controller.prototype.deployDNS = async function() {
    const podResource = this.resources.pods.dns;
    ...
    try {
        await this.deployPod(this.paths.kube.pods, this.paths.resources.pods.dns, podResource);
    } catch (err) {
...
Controller.prototype.reloadPods = async function(pods) {
    ...
    const reloadDNS = (
        ...
        kubeConfig.safeSearchEnforced !== savedConfig.safeSearchEnforced
    );
    ...
    try {
        ...
        await this.deployPod(this.paths.kube.pods, this.paths.resources.pods.dns, this.resources.pods.dns);
        await this.deployPod(this.paths.kube.pods, this.paths.resources.pods.transocks, this.resources.pods.transocks);
...
Controller.prototype.tearDown = async function() {
    ...
    await this.kubeDeleteIfExists(this.paths.resources.pods.dns);
    await this.kubeDeleteIfExists(this.paths.resources.pods.transocks);
...
Controller.prototype.addOrDeleteRules = async function(current, old, rules) {
    if (current && !old) {
        // Create rules
        for(let i =0; i < rules.length; i++) {
            ...
            await iptabler(rule).exec(err => {
                if (err) {
                    console.error(`Error applying rule ${rule} :: ${err.message}`)
                }
            });
        }
    } else if (!current && old) {
        ...
        for(let i = 0; i < rules.length; i++) {
            let rule = rules[i];
            let deleteRule = Object.assign({}, rule);
            deleteRule.delete = deleteRule.append;
            delete deleteRule.append;
            await iptabler(deleteRule).exec(err => {
                if (err) {
                    console.log(`Error deleting rule ${deleteRule} :: ${err.message}`)
                }
            });
        }
    }
 ...
 */
Controller.prototype.updateFirewall = async function() {
    const currentConfig = kubeData.config.networkConfig;
    const previousConfig = savedData.config.networkConfig;
    if (currentConfig) {
        if (!previousConfig || JSON.stringify(currentConfig) !== JSON.stringify(previousConfig)) {
            // Add/delete local rules
            await this.addOrDeleteRules(currentConfig.local, previousConfig.local, this.firewallRules.local);
        }
    }
}
