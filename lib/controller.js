...
function Controller(kubeConfig, userInfo) {
    ...
    this.resources = {
        ...
        daemonsets: {
            ...
            dns: readResourceFile(`${__dirname}/json/dns-daemonset.json`)
        },
        deployments: {
...
Controller.prototype.generatePaths = function(namespace) {
    ...
    this.paths.resources.daemonsets = {
        ...
        dns: `${this.paths.kube.daemonsets}/dns`
    };
...
Controller.prototype.updateDnsIp = async function() {
    try {
        const dnsInfo = (await this.kubeGet(this.paths.resources.services.dns)).body;
...
Controller.prototype.deployDNS = async function() {
    const daemonsetResource = this.resources.daemonsets.dns;
    ...
    try {
        step = 'daemonset';
        await this.deployDaemonset(this.paths.resources.daemonsets.dns, daemonsetResource);
        step = 'service'
        await this.kubeApplyService(this.paths.kube.services, this.paths.resources.services.dns, serviceResource);
    } catch (err) {
        throw new Error(`Failed to deploy DNS ${step}: ${err.message}`);
    }
...
Controller.prototype.tearDown = async function() {
    ...
    await this.kubeDeleteIfExists(this.paths.resources.daemonsets.dns);
...
Controller.prototype.applyRules = async function(rules) {
    for (let i = 0; i < rules.length; i++) {
        await iptabler(rules[i]).exec(err => {
            if (err) {
                const ruleStr = JSON.stringify(rules[i], null, 2);
                console.error(`Error applying rule ${ruleStr} :: ${err.message}`)
            }
        });
    }
}

Controller.prototype.flushChain = async function(chain) {
    await iptabler({
        sudo: true,
        table: 'nat',
        flush: chain
...
Controller.prototype.flushAllChains = async function() {
    await this.flushChain('GUARDIAN-LOCAL');
    await this.flushChain('GUARDIAN-GATEWAY');
    await this.flushChain('GUARDIAN-FORWARD');
    await this.flushChain('GUARDIAN-POSTROUTING');
}

Controller.prototype.initFirewall = async function(config) {
    await this.applyRules(this.firewallRules.createAll);
    await this.flushAllChains();
    // If firewall is configured, apply rules now
    await this.updateFirewall(config);
}

Controller.prototype.setInterfaces = function(rules, config) {
    const interfaceMap = {
        WAN: config.wan,
        LAN: config.lan,
        WIFI: config.wlan
    ...
    rules.forEach(rule => {
        if(rule.in_interface) {
            rule.in_interface = interfaceMap[rule.in_interface];
        }
        if(rule.out_interface) {
            rule.out_interface = interfaceMap[rule.out_interface];
        }
    });
}

Controller.prototype.updateFirewall = async function(config) {
    const currentConfig = config.networkConfig;

    await this.flushAllChains();
    if (currentConfig.local) {
        // Populate GUARDIAN-LOCAL chain
        await this.applyRules(this.firewallRules.local);

        if (currentConfig.gateway || currentConfig.wifi || currentConfig.vpn) {
            console.info('Could not apply additional firewall modes; must disable local first.')
        }
    } else {
        const interfaces = shelljs.ls('/sys/class/net');
        const wan = currentConfig.wan;
        if (currentConfig.gateway || currentConfig.wifi || currentConfig.vpn) {
            // Add postrouting rule
            if (interfaces.indexOf(wan) < 0) {
                console.warn(`Could not apply masquerade rules; '${wan}' interface is missing.`);
            } else {
                let egress = this.firewallRules.egress;
                this.setInterfaces(egress, currentConfig);
                await this.applyRules(egress);
            }
        }

        if (currentConfig.gateway) {
            const lan = currentConfig.lan;
            if (interfaces.indexOf(lan) < 0 && interfaces.indexOf(wan) < 0) {
                console.info(`Could not apply gateway rules; either '${lan}' or '${wan}' interface(s) missing.`);
            } else {
                let gatewayRules = this.firewallRules.wired;
                this.setInterfaces(gatewayRules, currentConfig);
                gatewayRules[0].source = config.gatewayIP;
                await this.applyRules(gatewayRules);
            }
        }
    }
    // TODO: apply gateway rules
}

Controller.prototype.tearDownFirewall = async function() {
    await this.flushAllChains();
    await this.applyRules(this.firewallRules.deleteAll)
}
